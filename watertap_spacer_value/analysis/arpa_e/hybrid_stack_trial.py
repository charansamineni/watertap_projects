...
from idaes.core.util import scaling as iscale, model_statistics as istat
...
def main():
    ...
    m.fs.product.report()
    print(
        f"Concentration of Li in Feed : {value(m.fs.feed.properties[0].conc_mass_phase_comp['Liq', 'Li_+'])} kg/m3"
    )
    print(
        f"Concentration of Li in Product : {value(m.fs.product.properties[0].conc_mass_phase_comp['Liq', 'Na_+'])} kg/m3"
    )
...
def build():
    ...
    ion_dict = {
        ...
        "mw_data": {"H2O": 18e-3, "Li_+": 23e-3, "Na_+": 23e-3, "Cl_-": 35.5e-3},
    ...
    m.fs.costing.electrodialysis.membrane_capital_cost.fix(
        250
    )  # Change the cost to your desired value
    m.fs.costing.electrodialysis.factor_membrane_replacement.fix(
        0.15
    )  # Change the replacement factor to your desired value
    ...
    m.fs.s04 = Arc(source=m.fs.EDstack.outlet_diluate, destination=m.fs.disposal.inlet)
    m.fs.s05 = Arc(
        source=m.fs.EDstack.outlet_concentrate, destination=m.fs.product.inlet
    )
...
def set_operating_conditions(m):
    ...
    m.fs.feed.properties[0].flow_mol_phase_comp["Liq", "Li_+"].fix(0.0045)
    m.fs.feed.properties[0].flow_mol_phase_comp["Liq", "Na_+"].fix(0.263)
    m.fs.feed.properties[0].flow_mol_phase_comp["Liq", "Cl_-"].fix(0.267)
    ...
    m.fs.EDstack.membrane_thickness["cem"].fix(1e-3)

    m.fs.EDstack.solute_diffusivity_membrane["cem", "Li_+"].fix(1.8e-10)
    m.fs.EDstack.solute_diffusivity_membrane["cem", "Na_+"].fix(1.8e-10)
    m.fs.EDstack.solute_diffusivity_membrane["cem", "Cl_-"].fix(1.8e-10)

    m.fs.EDstack.solute_diffusivity_membrane["aem", "Li_+"].fix(1.25e-10)
    ...
    m.fs.EDstack.ion_trans_number_membrane["cem", "Na_+"].fix(0.005)
    m.fs.EDstack.ion_trans_number_membrane["aem", "Na_+"].fix(0)
    m.fs.EDstack.ion_trans_number_membrane["cem", "Li_+"].fix(0.995)
    m.fs.EDstack.ion_trans_number_membrane["aem", "Li_+"].fix(0)
    ...
    print(istat.degrees_of_freedom(m))
    # check zero degrees of freedom
    assert istat.degrees_of_freedom(m) == 0, "Model has degrees of freedom!"
...
def optimize_system(m, solver=None, checkpoint=None, fail_flag=True):
    ...
    m.fs.EDstack.cell_pair_num.setlb(100)
    ...
    # m.fs.product.properties[0].conc_mass_phase_comp["Liq", "Li_+"].fix(2)
...
def display_model_metrics(m):
    ...
    pm_table = DataFrame(
        ...
        index=[
            ...
            "Concentration of Li in Product / Concentration of Li in Feed",
            "Selectivity of Li+ over Na+",
            "Concentration of Li in product, kg/m3",
        ],
    )
    print(pm_table)
