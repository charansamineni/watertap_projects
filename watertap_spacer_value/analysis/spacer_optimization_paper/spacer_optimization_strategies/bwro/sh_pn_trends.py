...
from watertap_spacer_value.flowsheets.ro_flowsheet_utils import (
    build_bwro_flowsheet as build_model, fix_model, scale_model, initialize_model
)
from watertap_spacer_value.flowsheets.ro_flowsheet_utils import (
    add_costing,
    collect_micro_trend,
)
from watertap_spacer_value.analysis.spacer_optimization_paper.ro_optimization_strategies.ro_spacer_process_optimization import solve, solve_for_recovery
...
def compare_strategies(correlation_type="guillen"):
    ...
    for strategy in strategies:
        m = init_build_bwro_flowsheet(nfe=60, correlation_type=correlation_type)
        ...
        macro_metrics.append({
            ...
            "Sh factor": [stage.feed_side.Sh_improvement_factor.value for stage in m.fs.ro.values()],
            "Friction factor": [stage.feed_side.friction_factor_improvement.value for stage in m.fs.ro.values()],
            "Channel height (mm)": [stage.feed_side.channel_height.value * 1e3 for stage in m.fs.ro.values()],
            "Spacer porosity": [stage.feed_side.spacer_porosity.value for stage in m.fs.ro.values()],
            "Membrane length (m)": [stage.length.value for stage in m.fs.ro.values()],
            "Pressure vessels": [stage.n_pressure_vessels.value for stage in m.fs.ro.values()],
            "Inlet velocity (cm/s)": [stage.feed_side.velocity[0, 0].value * 1e2 for stage in m.fs.ro.values()],
            "Membrane area (m2)": [stage.area.value for stage in m.fs.ro.values()],
        })

    macro_df = pd.DataFrame(macro_metrics).set_index("Strategy")

    with pd.ExcelWriter(f"sh_pn_trends_{correlation_type}.xlsx") as writer:
...
def init_build_bwro_flowsheet(nfe=60, correlation_type="guillen"):
    m = build_model(correlation_type=correlation_type, nfe=nfe)
...
if __name__ == "__main__":
    for corr in ["schock"]:
        print(f"Running for correlation: {corr}")
        compare_strategies(correlation_type=corr)
